...
Thread.sleep(1000) // 1 segundo de delay
    val document =
              app.get(
                "$mainUrl/index/search/q/${query.replace(" ", "%20")}",
                headers = baseHeaders  // ← Aquí agregamos los headers
            ).document

    return document.select("div.section-left div.capnhat div.section-bottom > div:nth-child(1)").mapNotNull { parent ->
